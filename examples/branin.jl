...
function branin(x1, x2; a=1, b=5.1 / (4π^2), c=5 / π, r=6, s=10, t=1 / (8π), noiselevel=0)
    x1 += Δ
    x2 += Δ
...
function minima(::typeof(branin))
    return [[-π - Δ, 12.275 - Δ], [π - Δ, 2.275 - Δ], [9.42478 - Δ, 2.475 - Δ]], 0.397887
end
mins, fmin = minima(branin)

function p()
    plt = contour(-15:0.1:15, -15:0.1:15, (x, y) -> -branin([x, y]); levels=80, fill=true)
    plt = scatter!(
        (x -> x[1]).(history(oh)[1]), (x -> x[2]).(history(oh)[1]); label="eval. hist"
    )
    plt = scatter!(
        (x -> x[1]).(mins),
        (y -> y[2]).(mins);
    ...
    plt = scatter!(
        [solution(oh)[1][1]], [solution(oh)[1][2]]; label="observed min.", shape=:rect
    )
    return plt
end
...
# profile optimization in VSCode
