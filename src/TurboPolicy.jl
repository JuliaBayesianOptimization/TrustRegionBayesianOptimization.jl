...
function AbstractBayesianOptimization.next_batch!(policy::TurboPolicy, dsm::Turbo{D, R},
    oh) where {D <: Real, R <: Real}
    @assert D == domain_eltype(oh)
    @assert R == range_type(oh)

    next_points = Vector{Vector{D}}(undef, dsm.batch_size)
    for j in 1:(dsm.batch_size)
        ...
        for i in 1:(dsm.n_surrogates)
            tr_xs = turbo_policy_seq(dsm, policy.candidate_size,
                policy.prob_of_perturbation, dimension(oh), i)
...
function turbo_policy_seq(dsm::Turbo{D}, candidate_size, prob_of_perturbation, dimension,
    i) where {D}
    xs = Vector{Vector{D}}()
